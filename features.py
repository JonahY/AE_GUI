...
class Features:
    ...
    def cal_PDF(self, tmp_origin, tmp_1, tmp_2, xlabel, ylabel, LIM=[[0, None]] * 3,
                INTERVAL_NUM=[6] * 3, color='black', FIT=False, select=[0, 1]):
        ...
        ax = fig.add_subplot()
        TMP, COLOR, LABEL = [tmp_origin, tmp_1, tmp_2], [color, self.color_1, self.color_2], \
        ...
        for tmp, color, label, num, lim in zip(TMP[select[0]:select[1]], COLOR[select[0]:select[1]],
            ...
            with open('/'.join([self.output, self.status]) + '_{}_'.format(label) + ylabel + '.txt', 'w') as f:
    ...
    def cal_CCDF(self, tmp_origin, tmp_1, tmp_2, xlabel, ylabel, LIM=[[0, float('inf')]] * 3,
                 color='black', FIT=False, select=[0, 1]):
        ...
        fig.text(0.15, 0.2, self.status, fontdict={'family': 'Arial', 'fontweight': 'bold', 'fontsize': 12})
        ax = fig.add_subplot()
        TMP, N, COLOR, LABEL = [tmp_origin, tmp_1, tmp_2], [N_origin, N1, N2], [color, self.color_1, self.color_2], \
                               ['Whole', 'Population 1', 'Population 2']
        for tmp, N, color, label, lim in zip(TMP[select[0]:select[1]], N[select[0]:select[1]],
            ...
            with open('/'.join([self.output, self.status]) + '_{}_'.format(label) + 'CCDF(%s).txt' % xlabel[0], 'w') as f:
                f.write('{}, {}\n'.format(xlabel, ylabel))
                for j in range(len(xx)):
                    f.write('{}, {}\n'.format(xx[j], yy[j]))
        plot_norm(ax, xlabel, ylabel, legend_loc='upper right')
    ...
    def cal_ML(self, tmp_origin, tmp_1, tmp_2, xlabel, ylabel, color='black', select=[0, 1]):
        N_origin, N1, N2 = len(tmp_origin), len(tmp_1), len(tmp_2)
        ...
        COLOR, LABEL = [color, self.color_1, self.color_2], ['Whole', 'Population 1', 'Population 2']
        for tmp, N, layer, color, label in zip(TMP[select[0]:select[1]], N[select[0]:select[1]],
            ...
            with open('/'.join([self.output, self.status]) + '_{}_'.format(label) + 'ML(%s).txt' % xlabel[0], 'w') as f:
                f.write('{}, {}, Error bar\n'.format(xlabel, ylabel))
    ...
    def cal_contour(self, tmp_1, tmp_2, xlabel, ylabel, x_lim, y_lim, size_x=40, size_y=40,
                    method='Linear bin', padding=False, colorbar=False):
    ...
    def plot_correlation(self, tmp_1, tmp_2, xlabel, ylabel, color='black', cls_1=None, cls_2=None, idx_1=None, idx_2=None,
                         fit=False, status='A-D', x1_lim=None, x2_lim=None, plot_lim=None):
    ...
    def plot_feature_time(self, tmp, ylabel, x_max=29000, color_tmp='black', color_stretcher='r', width=55,
        ...
        if stretcher_data:
            ...
            plot_norm(ax3, 'Strain (%)', x_lim=[0, strain_max], y_lim=[0, 15000], legend=False, font_color=color_stretcher)
        else:
            ax.bar(self.Time, tmp, color=color_tmp, width=width, log=True)
            plot_norm(ax, 'Time (s)', ylabel, x_lim=[0, x_max], y_lim=[0, 15000], legend=False)
        return plotWindow

    def cal_BathLaw(self, tmp_origin, tmp_1, tmp_2, xlabel, ylabel, interval_num, color='black', select=[0, 1]):
        ...
        ax = fig.add_subplot()
        TMP, MARKER, COLOR, LABEL = [tmp_origin, tmp_1, tmp_2], ['o', 'p', 'h'], [color, self.color_1,
                                                                                  self.color_2], ['Whole',
                                                                                                  'Population 1',
                                                                                                  'Population 2']
    ...
    def cal_WaitingTime(self, time_origin, time_1, time_2, xlabel, ylabel, interval_num, color='black', select=[0, 1]):
        ...
        TIME, MARKER, COLOR, LABEL = [time_origin, time_1, time_2], ['o', 'p', 'h'], \
                                     [color, self.color_1, self.color_2], ['Whole', 'Population 1', 'Population 2']
    ...
    def cal_OmoriLaw(self, tmp_origin, tmp_1, tmp_2, xlabel, ylabel, interval_num, select=[0, 1]):
        eny_lim = [[0.01, 0.1], [0.1, 1], [1, 10], [10, 100], [100, 1000]]
        tmp_origin = self.cal_OmiroLaw_helper(tmp_origin, eny_lim)
