...
class Preprocessing:
    ...
    def save_features(self, result):
        ...
        for idx, i in enumerate(result):
            ...
            print("Exporting Data: {}/{}".format(idx + 1, self.processor))
    ...
    def read_pac_data(self, file_name, tra_1=[], tra_2=[], tra_3=[], tra_4=[]):
        show_file_name = pow(10, len(file_name) - 2)
        for idx, name in enumerate(file_name):
            with open(name, "r") as f:
                self.skip_n_column(f)
                self.sample_interval = float(f.readline()[29:])
            ...
            if idx % show_file_name == 0:
                print('Loading waveforms... | Process: %d | Status: %d/%d' % (self.idx, idx + 1, len(file_name)))
        return tra_1, tra_2, tra_3, tra_4

    def read_pac_features(self, res, min_cnts=2):
        pri, chan_1, chan_2, chan_3, chan_4 = [], [], [], [], []
        show_res = pow(10, len(res) - 2)
        for idx, i in enumerate(res):
            ...
            if int(ls[-1]) > min_cnts:
                ...
                if int(ls[2]) == 1:
                    ...
                    chan_4.append(tmp)
            if idx % show_res == 0:
                print('Loading features... | Process: %d | Status: %d/%d' % (self.idx, idx + 1, len(res)))
...
def convert_pac_data(file_list, data_path, processor, threshold_dB, magnification_dB, load_wave=False, load_features=False):
    # check existing file
